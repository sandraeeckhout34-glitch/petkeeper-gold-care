...
export const Route = createFileRoute("/_authenticated/archived-pets")({
  head: () => ({ meta: [{ title: "Gearchiveerde huisdieren — PetKeeper" }] }),
  component: ArchivedPetsPage,
});
...
function Section({ title, icon: Icon, items, empty }: { title: string; icon: any; items: any[]; empty: string }) {
  return (
    <div className="mb-8">
      ...
      {items.length === 0 ? (
        <div className="bg-card rounded-3xl border border-border shadow-[var(--shadow-soft)] py-8 text-center text-sm text-muted-foreground">{empty}</div>
      ) : (
            ...
            <Link key={p.id} to="/pets/$id" params={{ id: p.id }} className="flex items-center gap-4 px-4 py-3 hover:bg-secondary/40 transition">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground truncate">
                  {p.status === "deceased" && p.deceased_date
                    ? `Overleden op ${p.deceased_date}`
